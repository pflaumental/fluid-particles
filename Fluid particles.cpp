...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
    ...
    int s = 3;
    g_nPaticles = s*s*s;
    g_pvParticle = new D3DXVECTOR3[g_nPaticles];
    for(int i=0; i<s; i++) {
        for(int j=0; j<s; j++) {
            for(int k=0; k<s; k++) {
                g_pvParticle[i*s*s + j*s + k] = D3DXVECTOR3(s-i*0.5f, s-j*0.5f, s-k*0.5f);
            }
        }
    }
...
void CALLBACK FP_OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{   
    g_GUI.OnGUIEvent(nEvent, nControlID, pControl, g_nActiveLight, g_nNumActiveLights, g_fLightScale);   
}
...
void CALLBACK FP_OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    V( g_pLightDir->SetRawValue( g_GUI.m_vLightDir, 0, sizeof(D3DXVECTOR3)*FP_MAX_LIGHTS ) );
    V( g_pLightDiffuse->SetFloatVectorArray( (float*)g_GUI.m_vLightDiffuse, 0, FP_MAX_LIGHTS ) );
    ...
    g_GUI.OnD3D10FrameRender(pd3dDevice, fTime, fElapsedTime, g_Camera.GetEyePt(),
            &mWorldViewProjection, &mWorld, &mView, &mProj, g_nNumActiveLights,
